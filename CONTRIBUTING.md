...
- Keep shortcuts out of labels/translations. Put command shortcut tokens and keyboard bindings in `packages/react/src/editor/commands/registry.ts`, then render them with `formatShortcut()` so macOS and Windows/Linux display correctly.
- Canvas context-menu grouping belongs in `packages/react/src/editor/menu-model/canvas.ts`; components should render the menu model instead of hand-building command groups.
