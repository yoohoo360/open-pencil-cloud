...
/** While the shared canvas loading overlay is up, fill larger contiguous areas. */
const LOADING_TILE_FRAME_BUDGET_MS = 28
const LOADING_MAXIMUM_TILE_JOBS_PER_FRAME = 96
const LOADING_TILE_OVERSCAN = 4
...
export class TiledSceneController {
  ...
  renderFrame(
    ...
    const loadingBoost = renderer.tiledSceneLoadingBoost
    const frameBudgetMs = loadingBoost ? LOADING_TILE_FRAME_BUDGET_MS : TILE_FRAME_BUDGET_MS
    const maximumJobsPerFrame = loadingBoost
      ? LOADING_MAXIMUM_TILE_JOBS_PER_FRAME
      : MAXIMUM_TILE_JOBS_PER_FRAME
    const overscanTiles = loadingBoost ? LOADING_TILE_OVERSCAN : TILE_OVERSCAN
  ...
  private runScheduledFrame(
    ...
    frameOptions?: { budgetMs?: number; maximumJobsPerFrame?: number }
  ): TileSchedulerMetrics {
    const budgetMs = frameOptions?.budgetMs ?? TILE_FRAME_BUDGET_MS
    return this.scheduler.runFrame(
      (job) => this.executeJob(renderer, graph, index, job, budgetMs),
      frameOptions
    )
  }
  ...
  private executeJob(
    ...
    index: RenderChunkIndex,
    job: TileJob,
    frameBudgetMs = TILE_FRAME_BUDGET_MS
  ) {
    ...
    if (
      ...
      return this.jobResult(tile, frameBudgetMs)
    }
  ...
  private jobResult(
    tile: NonNullable<ReturnType<typeof renderTile>>,
    frameBudgetMs = TILE_FRAME_BUDGET_MS
  ) {
    return {
      renderMs: tile.renderMs,
      overBudget: tile.renderMs > frameBudgetMs,
