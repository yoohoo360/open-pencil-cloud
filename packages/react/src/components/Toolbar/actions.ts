...
export function useToolbarActions({ getCommand, menu }: ToolbarActionOptions) {
  const editActions: ToolbarActionItem[] = [
    { icon: Copy, label: menu.copy, action: () => {} },
    { icon: Clipboard, label: menu.paste, action: () => {} },
    { icon: Scissors, label: menu.cut, action: () => {} },
    {
      icon: CopyPlus,
