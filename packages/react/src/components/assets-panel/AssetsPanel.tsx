import { BookOpen, ChevronLeft, Component, LayoutGrid, List, Plus } from 'lucide-react'
...
export function AssetsPanel() {
  ...
  const [activeLibKey, setActiveLibKey] = useState('')
          ...
          <div className="space-y-1">
            {libraries.map((libItem) => (
              <button
                ...
                onClick={() => setActiveLibKey(libItem.key)}
              >
                {libItem.remote ? (
        ...
        {activeLibKey
          ? assetGroups.map((group) => (
              <section key={group.pageId} className="mb-3">
                <h2 className="mb-1 px-1 text-[10px] font-medium tracking-wide text-muted uppercase">
                  {group.pageName}
                </h2>
                <div
                    ...
                    return (
                      ...
                      >
                        {asset.componentId && sourceLibraryKey ? (
                          ...
                          <Icon className="size-4 shrink-0 text-component" aria-hidden="true" />
                        )}
