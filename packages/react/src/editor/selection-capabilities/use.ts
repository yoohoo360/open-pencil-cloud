...
export function useSelectionCapabilities() {
  ...
  const selectedNodesCanFlatten = useSceneComputedRef(() => {
  ...
  return {
    ...
    canOutlineText: useSceneComputedRef(() => {
      const nodes = editor.getSelectedNodes()
      return (
        nodes.length > 0 &&
    ...
    canOutlineStroke: useSceneComputedRef(() => {
      const nodes = editor.getSelectedNodes()
      return (
        nodes.length > 0 &&
        nodes.every(
    ...
    canMoveToPage: useSceneComputedRef(() => hasSelection.value && editor.graph.getPages().length > 1),
    canSelectAll: useSceneComputedRef(
      () => editor.graph.getChildren(editor.state.currentPageId).length > 0
    ),
    canUndo: useSceneComputedRef(() => editor.undo.canUndo),
    canRedo: useSceneComputedRef(() => editor.undo.canRedo),
