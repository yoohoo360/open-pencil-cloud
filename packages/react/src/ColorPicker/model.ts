...
/** RGB color in 0–255 channel space with alpha 0–1 (reka-ui compatible shape). */
...
/** HSL color with h in degrees, s/l in percent, alpha 0–1. */
...
/** HSB/HSV color with h in degrees, s/b in percent, alpha 0–1. */
...
export type PickerColor = RGBColor | HSLColor | HSBColor

export interface ColorPickerModel {
  /** RGB representation kept under the historical `rekaColor` name for API parity. */
  rekaColor: RGBColor
  rgb: RGBColor
  hsl: HSLColor
  hsb: HSBColor
...
/** Converts a picker color (rgb/hsl/hsb) to an app Color. Kept name for API parity. */
export function rekaToAppColor(color: PickerColor): Color {
