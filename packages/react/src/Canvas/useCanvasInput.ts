...
import type { SceneNode, VectorSegment } from '@open-pencil/scene-graph'
...
export function useCanvasInput(
  ...
  function handleTextEditClick(cx: number, cy: number, shiftKey: boolean): boolean {
    ...
    if (clickCount >= 3) {
      ...
      drag.current = { type: 'text-select', startX: cx, startY: cy } as DragState
    }
  ...
  function onMouseDown(e: MouseEvent) {
    ...
    drag.current = { type: 'draw', startX: cx, startY: cy, nodeId }
  }
  ...
  function onMouseMove(e: MouseEvent) {
    ...
    if (editor.state.activeTool === 'PEN' && editor.state.penState && !drag.current) {
      const { cx, cy } = getCoords(e)
    ...
    if (!drag.current && editor.state.activeTool === 'SELECT') {
      const { sx, sy, cx, cy } = getCoords(e)
      setCursorOverride(updateHoverCursor(sx, sy, cx, cy, editor, hitFns))
    }
