...
export function GradientEditorRoot({ fill, onUpdate, children }: GradientEditorRootProps) {
  ...
  const stops = useMemo<GradientStop[]>(() => fill.gradientStops ?? [], [fill.gradientStops])
  const subtype = fill.type as GradientSubtype

  const activeColor = useMemo<Color>(() => {
    if (!stops.length) return fill.color
  ...
  function removeStop(index: number) {
    ...
    setActiveStopIndex((prev) => Math.min(prev, stops.length - 2))
  }
