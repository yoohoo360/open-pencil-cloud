...
export function createStrokeGeometryState({ nodes, merged }: StrokeGeometryStateInput) {
  ...
  const cap = useMemo(() => merged('strokeCap'), [nodes, merged])
  const join = useMemo(() => merged('strokeJoin'), [nodes, merged])
  const miterLimit = useMemo(() => merged('strokeMiterLimit'), [nodes, merged])

  return { advancedActive, cap, join, miterLimit }
}
...
export function createStrokeSideActions(
  editor: Editor,
  sideMenuOpen: MutableRefObject<boolean>,
  closeSideMenu?: () => void
) {
