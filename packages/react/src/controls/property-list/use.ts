...
export function useEditorPropertyList<K extends PropertyListKey>(propKey: K) {
  ...
  const activeNode = useSceneComputed<SceneNode | null>(() => selectedNodes[0] ?? null)
  const selectedNodeIds = useMemo(() => selectedNodes.map((node) => node.id), [selectedNodes])
  const isMulti = selectedNodes.length > 1
  const active = selectedNodes.length > 0
  const isMixed = useMemo(() => isArrayMixed(propKey), [isArrayMixed, propKey, selectedNodes])
  const items = useSceneComputed<PropertyListItemFor<K>[]>(() => {
    void editor.state.sceneVersion
    if (isArrayMixed(propKey)) return []
    return (activeNode?.[propKey] ?? []) as PropertyListItemFor<K>[]
  })
  ...
  function add(item: PropertyListItemFor<K>) {
    ...
    const label = isMulti ? `Set ${propKey}` : `Add ${propKey}`
    const apply = () => {
      for (const node of nodes) {
        const nextItem = structuredClone(item)
        const next = isMulti ? [nextItem] : [...propArray(node), nextItem]
