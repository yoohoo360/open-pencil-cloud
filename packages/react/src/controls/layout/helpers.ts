import { computed, ref } from '#react/internal/reactive'
...
export function createLayoutSelectionState(
  ...
  const node = useSceneComputedRef<SceneNode | null>(() => editor.getSelectedNode() ?? null)
  const layoutDirection = computed<SceneNode['layoutDirection']>(
    () => node.value?.layoutDirection ?? 'AUTO'
  )
  const sizingState = createLayoutSizingState(editor, node, { value: panels })
...
export function createTrackSizingOptions(panels: ReturnType<typeof useI18n>['panels']) {
  return [
