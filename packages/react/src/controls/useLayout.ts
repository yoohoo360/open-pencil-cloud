...
function createTrackSizingOptions(panels: ReturnType<typeof useI18n>['panels']) {
  return [
...
export function useLayout() {
  ...
  const widthSizingOptions = (() => {
    const options: { value: LayoutSizing; label: string }[] = [
      { value: 'FIXED', label: panels.sizingFixed }
    ]
    if (isFlex) options.push({ value: 'HUG', label: panels.sizingHug })
    if (isInAutoLayout || isFlex) options.push({ value: 'FILL', label: panels.sizingFill })
    return options
  })()

  const heightSizingOptions = (() => {
    const options: { value: LayoutSizing; label: string }[] = [
      { value: 'FIXED', label: panels.sizingFixed }
    ]
    if (isFlex) options.push({ value: 'HUG', label: panels.sizingHug })
