...
export class SceneGraph {
  ...
  private registerNode(node: SceneNode, parentId: string | null): SceneNode {
    ...
    if (this.silentMutationDepth === 0) this.emitter.emit('node:created', node)
    return node
  }
  ...
  get isSilentMutation(): boolean {
    return this.silentMutationDepth > 0
  }
  ...
  updateNode(id: string, changes: Partial<SceneNode>): void {
    ...
    if (this.silentMutationDepth === 0) this.emitter.emit('node:updated', id, changes)
  }
  ...
  reorderChild(nodeId: string, parentId: string, insertIndex: number): void {
    ...
    if (this.silentMutationDepth === 0) this.emitter.emit('node:reordered', nodeId, parentId, idx, previousParentId)
  }
  ...
  insertChildAt(childId: string, parentId: string, index: number): void {
    ...
    if (this.silentMutationDepth === 0) this.emitter.emit('node:reordered', childId, parentId, index, previousParentId)
  }
  ...
  deleteNode(id: string): void {
    ...
    if (this.silentMutationDepth === 0) this.emitter.emit('node:deleted', id, node.parentId)
  }
